...
document.getElementById('login').addEventListener('click', async () => {
    ...
    if (res.error) {
        ...
        if (res.data.user) {
            ...
            $loginStatus.innerHTML = `로그인된 id: ${JSON.stringify(res.data.user)}`;
        }
    }
});
...
document.getElementById('logout').addEventListener('click', async () => {
    ...
    const $logoutStatus = document.getElementById('login-status');
    $logoutStatus.innerHTML = `로그아웃되었습니다.`;
});
